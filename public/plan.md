...
medicos/{place_id}
  nombre
  especialidad                # efectiva: especialidad_normalizada ?? especialidad_raw. Es el campo que filtra /directorio
...
- `especialidad_normalizada`/`confidence` son metadata derivada del `nombre`. `/directorio` filtra por el campo **`especialidad`** = `especialidad_normalizada ?? especialidad_raw`: lo que el lugar aparenta ser gana sobre lo que buscamos. Sin esto la normalización quedaba escrita y nunca leída, y una "Clínica Pediátrica" hallada con un keyword de cardiología aparecía bajo cardiología.
- El fallback a `especialidad_raw` es deliberado: cuando el `nombre` no revela nada, la única señal disponible es el término con el que se buscó. Preferimos esa señal débil pero declarada a esconder el registro de todo filtro.
- Los documentos escritos antes de este cambio no tienen `especialidad` y no matchean el filtro hasta ser re-recolectados o purgados por el TTL de 30 días. Se optó por no escribir un script de backfill: la ventana se cierra sola.
...
- [x] Documentación técnica (máx. 5 páginas) — `docs/documentacion-tecnica.md`
- [x] Diagrama de arquitectura — Mermaid en `docs/documentacion-tecnica.md` §2
- [x] **Data Card** del dataset — `docs/data-card.md` (falta publicarla en `/datacard` + PDF)
- [x] Modelo de amenazas de la IP whitelist — `docs/documentacion-tecnica.md` §6
- [x] Sección "Postura ética" — `docs/documentacion-tecnica.md` §11
- [ ] Informe de auditoría de cobertura con números reales — metodología y generador listos
      (`docs/auditoria-cobertura.md` + `functions/scripts/coverage-report.js`); **bloqueado por
      la recolección en producción**
- [ ] Telemetría de costo: total, registros únicos, costo por registro — la calcula el mismo
      generador; bloqueada por lo mismo
